...
class LGTextSplitter(CharacterTextSplitter):
    ...
    def split_text(self, text: str) -> List[str]:
        ...
        for chunk in text:
            if len(chunks) < self._chunk_size:
                ...
                chunks = chunks[-self._chunk_overlap:]
                #判断chunks的第一个是不是中文
                while not re.search("[\u4e00-\u9fa5]", chunks[0]):
                    chunks = chunks[1:]
        chunks_list.append(text[-self._chunk_size:])
        return chunks_list

if __name__ == "__main__":
    text_splitter = LGTextSplitter(
        chunk_size=300,
        chunk_overlap=90
    )
    ls = [
